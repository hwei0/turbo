...
impl WeightedStreamManager {
    ...
    pub async fn read_zmq_socket_loop(
        ...
        // Startup step: Map the POSIX shared memory region that the Python process writes image data into.
        ...
        loop {
            ...
            // KEY DESIGN: Copy-before-ACK ordering. We memcpy from SHM before
            // sending the ACK. The ACK tells Python the SHM region is safe to overwrite.
            // If we ACKed first, Python could write the next frame while we're still
            // reading — a data race. This ordering is the safety argument for the
            // unsafe Send+Sync impls on the SHM pointer wrappers.
            unsafe {
                memcpy(
