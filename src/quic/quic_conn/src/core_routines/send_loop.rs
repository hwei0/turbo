...
impl WeightedStreamManager {
    ...
    pub async fn send_loop(
        ...
        // Staging buffer: collects (payload_bytes, context_id, is_final_chunk) tuples
        // before writing them to the QUIC stream. This separates the queue-draining
        // phase (which holds the queue lock) from the I/O phase (which does async writes).
        ...
        loop {
            ...
            // Compute the byte budget for this iteration:
            // tx_bytes = allocated_bw (bytes/sec) * elapsed_time (sec).
            // This enforces the bandwidth limit — we only send as many bytes as the
            // allocation allows for the elapsed wall-clock time since the last iteration.
            let mut tx_bytes = {
            ...
            while tx_bytes > 0 && !queue_vec.is_empty() {
                ...
                if queue_item.byte_data.len() as i64 > tx_bytes {
                    // PARTIAL SEND: payload exceeds remaining byte budget.
                    // Drain tx_bytes from the front of byte_data, advance tx_idx,
                    // and push the item back to the queue front for next iteration.
